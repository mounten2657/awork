 ...
 * @example http://www.awork.local/test/apifox/apifox2yapi.php?project=api&group=病历导出--pathology
...
{
    $url = $params['url'] ?: 'http://127.0.0.1:4523/export/openapi?projectId=412627';
    $project = $params['project'] ?: '';
    $regular = $params['regular'] ?: '';
    $group = $params['group'] ?: '';
    $group = array_filter(explode(',', $group, 2));
    if (empty($group)) {
    ...
    $res = str_replace("/api/v1", '/v1', $res);
    $res = str_replace("/msg/api/", '/message/api/', $res);
